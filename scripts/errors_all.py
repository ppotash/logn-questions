...
def classify(g, pool, cache):
    """Returns (category, first_error_round or None, all_error_rounds). Categories are CATS."""
    tid = g["target_id"]
    ...
    # Find ALL answer errors, not just the first one.
    err_rounds = []
    for h in g["history"]:
        ...
        if cache[k] != h["answer"]:
            err_rounds.append(h["round"])

    if g["won"]:
        return ("win+err", err_rounds[0], err_rounds) if err_rounds else ("win", None, [])
    if err_rounds:
        return ("answer", err_rounds[0], err_rounds)
    if not g["guess"]:
        return ("no guess", None, err_rounds)
    ...
    for h in g["history"]:
        ...
        if k not in cache:
            return ("unjudged", None, err_rounds)
        if cache[k] != h["answer"]:
            return ("prediction", None, err_rounds)
    return ("discrimination", None, err_rounds)
...
def report(pool, cache, label=""):
    ...
    all_answer_errors = defaultdict(int)
    all_answer_errors_by_size = defaultdict(int)
    for g in games():
        cat, rd, err_rounds = classify(g, pool, cache)
        by_model[g["model"]][cat] += 1
        by_size[g["size"]][cat] += 1

        # Count EVERY erroneous answer in the game.
        n_errors = len(err_rounds)
        all_answer_errors[g["model"]] += n_errors
        all_answer_errors_by_size[g["size"]] += n_errors

        # Keep the first error round for the "where target dies" diagnostic.
    ...
    print("\nTOTAL answer errors (every erroneous round, not games):")
    for m in sorted(all_answer_errors):
        print(f"  {m[:17]:<18}{all_answer_errors[m]:>5}")

    print("\nanswer errors by N (every erroneous round):")
    print(f"{'N':<18}{'errors':>9}")
    print("-" * 27)
    for n_ in sorted(all_answer_errors_by_size):
        print(f"{n_:<18}{all_answer_errors_by_size[n_]:>9}")

    print("\nanswer-error summary:")
    for m in sorted(all_answer_errors):
        affected = by_model[m]["answer"] + by_model[m]["win+err"]
        total_errs = all_answer_errors[m]
        avg = total_errs / affected if affected else 0.0
        print(f"  {m[:17]:<18} affected games={affected:<4} "
              f"total errors={total_errs:<4} avg/error-game={avg:.2f}")
    ...
    print("\nwhere the first target-answer error occurs, as a fraction of the round budget:")
...
def report_all(pool, cache_dir):
    ...
    error_totals = {j: defaultdict(int) for j in judges}
    error_sizes = {j: defaultdict(int) for j in judges}
    for g in games():
        for j, c in caches.items():
            cat, _, err_rounds = classify(g, pool, c)
            tallies[j][g["model"]][cat] += 1
            sizes[j][g["size"]][cat] += 1
            error_totals[j][g["model"]] += len(err_rounds)
            error_sizes[j][g["size"]] += len(err_rounds)

    print("\nanswer errors: affected games and total erroneous rounds, by judge")
    print(f"{'model':<18}" + "".join(f"{j[:8]:>9}" for j in judges)
          + f"{'self':>7}{'others':>9}{'all errs':>9}")
    print("-" * (18 + 9 * len(judges) + 25))
    for m in models:
        vals = {j: tallies[j][m]["answer"] + tallies[j][m]["win+err"]
                for j in judges}
        errs = {j: error_totals[j][m] for j in judges}
        ...
        total_other_errs = [errs[j] for j in judges if self_of[j] != m]
        eo = sum(total_other_errs) / len(total_other_errs) if total_other_errs else float("nan")
        print(f"{m[:17]:<18}" + "".join(f"{vals[j]:>9}" for j in judges)
              + f"{sv:>7}{mo:>9.1f}{eo:>9.1f}")

    print("\ntotal answer errors (every erroneous round), by judge")
    print(f"{'judge':<12}{'affected':>10}{'all errs':>10}{'err/game':>10}")
    print("-" * 42)
    for j in judges:
        affected = sum(tallies[j][m]["answer"] + tallies[j][m]["win+err"]
                        for m in models)
        all_errs = sum(error_totals[j][m] for m in models)
        avg = all_errs / affected if affected else 0.0
        print(f"{j[:11]:<12}{affected:>10}{all_errs:>10}{avg:>10.2f}")
