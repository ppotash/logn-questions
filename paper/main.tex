...
game of Potash and Suleman \cite{potash2019}. A questioner sees $N$ Wikipedia
...
reliability parameter, $\text{win}=p^{\log_2 N}$ with $p=0.928$. We read this
as evidence that scale degrades performance by adding failure opportunities,
not by making any individual inference harder.

We adjudicate each game's target and guess against three independent judges,
2{,}931 judgements each. Losses divide into answer errors and discrimination failures in roughly equal
measure, 47--55\% and 42--49\% depending on the judge, with 2--4\% prediction
errors. Models almost never name a document their own evidence excludes, and
the split does not shift detectably with set size. Measured per-round agreement rates, 0.900
to 0.994, reconcile with the fitted $p$; the residual is the discrimination
failures. Inter-judge agreement on individual judgements is 97.8--98.7\%. Information per question, estimated from answer balance, correlates
with win rate at $r=+0.88$. The only two models to extract a full bit per
question are the only two that partition on document titles, a strategy absent
below $N{=}32$ and used in a quarter of questions above it.

Reasoning-token expenditure varies $4.5\times$ across models with little
relation to success. Cost is uncorrelated with performance at $r=-0.05$: the
two cheapest arms win at \$0.36 and \$0.39 each, against \$2.46 for the most
expensive. Two China-hosted providers refused documents in our corpus. Moonshot rejected
three of the 1024 paragraphs, on Taiwan's navy, a purged Chinese intellectual,
and a detained Hong Kong activist; Z.ai rejected the third of these. We located
them by bisection and report the refusals as a practical limit on assembled
long contexts. Our per-model ordering inverts published
general-intelligence leaderboards, on which the model finishing last here ranks
at or near the top. \S\ref{sec:external} sets out where independent benchmarks
agree with our result and where they do not.

\medskip
\noindent\textbf{Scope.} This is an exploratory report, not a conference
submission. Every cell is eight games, so per-size comparisons are suggestive.
The durable findings are those pooled across sizes, the mechanisms identified
by hand-adjudication, and the engineering account.
\end{abstract}
...
\paragraph{What this report is.} The experiment is small and self-funded. Six
models, nine set sizes, eight games per cell, \$363 of API spend
(\S\ref{sec:cost}). The task probes three capacities that are
hard to isolate separately, the harness is reproducible and
released, and the failure modes we identify by hand-adjudication hold
regardless of sample size. A model whose questions select a third of the field
instead of a half is doing so for mechanical reasons that eight games suffice
to expose, even where the resulting win rate is imprecisely estimated. The
per-model ordering is a different matter, and \S\ref{sec:limits} says what it
rests on.
...
per round, validated across three judges, with a measured
self-preference discount of 40 to 46\% and inter-judge agreement of
97.8--98.7\%.
\item A taxonomy of inter-agent communication failures with observed instances
of each, and the finding that all four prompt revisions that improved play were
fixes to communication, not to either agent's reasoning.
...
Two mechanisms are often grouped together but behave differently here.
...
\citep{zhipu2026glm5} have published technical reports. And our results below
show partition-quality deficits that are present at $N{=}8$ as strongly as at
$N{=}1024$ (\S\ref{sec:aggregation}), which no attention-mechanism account
predicts. We therefore present this section as motivation for the task design,
not as a hypothesis the results confirm.
...
dominates for Opus, with a candidate explanation in disclosed training toward
strict document grounding. The figures below should therefore be read as an
upper bound on questioner-side deficit.
...
\bottomrule
\end{tabular}
\caption{Yes rate by round; 50\% indicates a balanced partition. Gap is
(rounds 2+) $-$ (round 1): positive means the model is better once the
candidate set has been constrained. $z$ is against a balanced null over the 68
...
survey. Two models, Grok and Opus, are substantially worse on round 1 and
recover afterwards. Opus's opening questions select 22\% of the corpus rather than
...
$0.035$; Opus hit that on six of nine sizes.

Round-1 balance is the better predictor of the two. Across the six arms, $H(p)$
from round 1 correlates with win rate at $r=+0.88$, from later rounds at
$r=+0.81$, and the round-1/later gap itself at $r=-0.75$. Six points cannot
...
\paragraph{The deficit does not scale with $N$.} The round-1 result invites a long-context reading. The opening question is the
only round that demands an unconstrained survey of the whole input
(\S\ref{sec:longctx}). The data rule this out. Opus's
...
$+6$ for Gemini --- suggesting round position matters independently. This comparison is confounded. The candidate count assumes even splits, which
is what Opus fails to achieve, so its true later-round candidate sets are
larger than the matched cells imply. Resolving it needs the partition
...
game that provides no slack. Neither context length nor candidate-set size accounts for it. The measurement itself is cheap, judge-free, and computable from
...
GPT--Grok), over roughly 2{,}900 shared pairs each. We do not read this as evidence that the judgements are correct. \citet{panickssery2024} show that
...
even though they are not losses; 5 to 9 such games occur per judge. Opus is
...
with losses, as an earlier version of our analysis did, inflates the loss total
by four percent and misattributes wins.
...
Answer errors and discrimination failures are comparable in size, and which is
larger depends on the judge: under Gemini and Grok the answer share leads,
under GPT it does not. We therefore treat them as roughly equal contributors
rather than ranking them.

\paragraph{Why one model's answerer fails so much more often.} Opus's 32--34
answer errors against Grok's 2.5 is the largest single per-model difference in
the study, and it has a plausible differential explanation in disclosed
training posture rather than in architecture.

The knowledge-conflict literature documents large per-model variation in how
strictly a model grounds an answer in provided context versus its own
parametric knowledge \citep{longpre2021,xie2024conflict}. Anthropic's public
posture is unusually far toward the grounding end: a Citations API built
specifically so that answers are anchored in source documents, hallucination
guidance instructing users to ``explicitly instruct Claude to only use
information from provided documents and not its general knowledge'', and a
published constitution setting an honesty bar its authors describe as
substantially higher than standard human ethical norms; the underlying method
is Constitutional AI \citep{bai2022cai}.

The claim has to be differential to do any work. Post-training damages
calibration in general: base models are well calibrated and RLHF policies much
less so \citep{kadavath2022}. But all six models here are post-trained, so a
generic account cannot explain why one sits at 22\% opening balance and another
at 49\%. What distinguishes Anthropic is the direction of its post-training,
not the fact of it.

That posture predicts the failure we observe. Asked whether the subject of
``Vocento, S.A.\ \dots is a Spanish mass media group'' is associated with a
country in Europe, a model trained toward document-only grounding answers No,
because the document does not say that Spain is in Europe. This is not a
reasoning failure. It is the correct output of a strict grounding rule applied
to a question requiring one inferential step outside the text.

Our own prompt history supplies evidence. The first version of the answerer
instruction told it to ``answer on the basis of the document alone''. Under
that wording, answer balance was 21\% Yes across 24 measured calls where
$\approx$50\% was expected ($p=0.003$). We replaced it with an explicit licence
to apply ordinary world knowledge and changed nothing else. The Vocento
judgement moved to Yes and the balance moved toward 50\% for every model. The
prompt fix was an unplanned ablation of this hypothesis. Some literalism
persists for Opus under the corrected wording, which is consistent with a
trained prior that one instruction only partly overrides.

This resolves an apparent tension between \S\ref{sec:bits} and this section. A
literal answerer that says ``No'' more often depresses the measured yes rate,
which mimics a questioner asking narrow questions. The two are
indistinguishable in the balance statistic and separable only by adjudication.
Our adjudication favours the answerer-side reading. Opus's 22\% opening balance
is therefore at least partly an artefact of its answerer, not an independent
questioning pathology.

\paragraph{Reconciling with the compounding fit.} \S\ref{sec:decay} fits
$\text{win}=p^{\log_2 N}$ with $p=0.928$ from outcomes alone. The per-round
agreement measured here is higher for every leading model, 0.967 to 0.994. The
two figures are consistent. Answer agreement alone is too high to explain the
observed decay, and the remainder is discrimination failure. The
outcome-fitted parameter sits below every measured agreement rate by roughly
the amount the 51/45 loss split implies, which is a consistency check between
two independent measurements.

\paragraph{The split does not shift with set size.}
...
Total losses grow with $N$, as the win-rate curve requires. The \emph{share}
attributable to each cause does not move detectably. Pooling $N\le32$ against
$N\ge256$ gives answer shares of 54\% and 48\% (Fisher exact, $p=0.82$). The
per-size shares run 70, 86, 45, 39, 63, 53, 46, 48 and 51 percent, which is
consistent with sampling noise around a constant. Individual cells are small:
$N{=}8$ contains seven losses in total, so its 6:1 ratio carries almost no
weight.

Prediction errors are the exception, being absent below $N{=}64$ and appearing
sporadically above it. Even there the counts are one or two games per size.
...
$11\times$: GPT 446, GLM 950, Opus 824, Kimi 2{,}644, Gemini 4{,}272, Grok
5{,}054. At $N{=}1024$ the spread is $4.5\times$: GPT 29{,}574, Opus 38{,}334,
Kimi 101{,}800, Gemini 103{,}286, GLM 110{,}674, Grok 134{,}138. At $N{=}512$ Grok emits 160{,}261 output tokens against GPT's 22{,}325 on the
same game; on one measured call 99.2\% of Grok's output was reasoning rather
than visible answer.
Gemini, run at MEDIUM for the reasons in \S\ref{sec:reasoning}, outperforms two
models run at HIGH. The data are inconsistent with a simple
more-reasoning-is-better account of the kind that motivates test-time compute
scaling \citep{wei2022cot,snell2024}, and are consistent with the
...
varies $4.5\times$ at $N{=}1024$ with little relation to success, and that Gemini at MEDIUM
...
driven by its failures: the world-knowledge licence came from its answer to the
...
producing it. Tuning against Opus should, if anything, have helped Opus.
...
Anthropic's disclosed training encourages, and Opus retains the highest answer
...
\paragraph{Testing the literalism account.} Run the answerer under two
instructions --- strict document-only grounding versus explicit world-knowledge
licence --- holding everything else fixed, and measure the change in answer-error
rate per model. Our prompt history supplies an accidental version of this
experiment (\S\ref{sec:errors}); a controlled one would establish whether the
per-model differences in answer error are a trained prior or a prompt artefact.
