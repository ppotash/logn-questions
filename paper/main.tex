...
failures. Inter-judge agreement on individual judgements is 97.8--98.7\%.
Inspecting every unanimous answer error from the weakest model finds 32 of 34
are ``No'' answers, on properties stated in the document's first sentence, under
an instruction that explicitly warns against defaulting to ``No''. Information per question, estimated from answer balance, correlates
...
dominates for Opus: 32 of its 34 unanimous answer errors are ``No'' answers on
...
the study. Inspecting all 34 of its unanimous errors under the frozen prompt
identifies the pattern, and rules out the explanation we first proposed.
...
\textbf{This falsifies a hypothesis we had proposed.} An earlier reading
attributed the gap to context-faithfulness: models differ in how strictly they
ground answers in provided text \citep{longpre2021,xie2024conflict}, and
Anthropic's public posture is unusually far toward the grounding end, with a
Citations API built so that answers are anchored in source documents,
hallucination guidance instructing users to restrict the model to provided
material, and a published constitution setting an elevated honesty bar
\citep{bai2022cai}. That account predicts refusal to make inferences \emph{one
step outside} the document. It does not predict contradicting the document's
own first sentence, which is what these errors do. We record it because the
prediction was specific enough to be wrong.

\textbf{What we can state.} One model's answerer carries a strong bias toward
``No'', which persists under an instruction that says verbatim: ``Yes and No
are equally acceptable answers: do not fall back on No when uncertain.'' The
bias is visible in three independent measurements: a 22\% round-1 yes rate
(\S\ref{sec:bits}), the lowest agreement rate of any model at $p=0.900$, and
the 32:2 direction of its errors. We have no mechanism to offer. Generic
accounts of post-training damaging calibration \citep{kadavath2022} apply to
all six models and cannot explain a difference between them, and the
architecture of this model is undisclosed (\S\ref{sec:limits}).

We note the bias is not confined to the weakest model: round-1 yes rates are
below 50\% for all six (49, 49, 44, 41, 34, 22\%), so some pull toward ``No''
appears general and differs across models by degree.
...
Europe?'' --- asked of the same document. Answered No, but under our first
answerer instruction, which forbade world knowledge (\S\ref{sec:prompts}).
Included to show the failure mode; it is not evidence about the model. \\
...
which forbade world knowledge; the round-2 answer is what that instruction
requires, and the game is shown to illustrate how a single answer propagates,
not as evidence about the model. Under the frozen prompt the same document
...
presupposed the candidate set. The revisions were: the world-knowledge licence came from its answer to the
