...
reliability parameter, $\text{win}=p^{\log_2 N}$ with $p=0.928$. That form
...
failures in roughly equal measure, 47--55\% and 42--49\% depending on the judge,
with 2--4\% prediction errors: models almost never name a document their own
evidence excludes, and the split does not shift detectably with set size.
...
long contexts. Our per-model ordering inverts published general-intelligence
...
The pooled drop is $z=2.78$, $p=0.005$. Only the Opus row is individually
...
\paragraph{The deficit does not scale with $N$.} The round-1 result invites a
long-context reading. The opening question is the
...
$+6$ for Gemini --- suggesting round position matters independently. This
comparison is confounded. The candidate count assumes even splits, which
...
the study. We inspected all 34 of its unanimous errors under the frozen prompt.
...
\paragraph{Two observations we flag as suggestive.} Opus spends 296 tokens on
...
six models this establishes nothing beyond the observation about Opus itself.
...
Self-correction & ``No\dots\ wait. The document concerns biathlon, a
winter sport. Yes'' --- the answerer's reflexive No, corrected only because it had room
...
its prompt or schema version differs from the current one. See \url{https://github.com/ppotash/logn-questions}.
